...
export const GetUsersByUserKeysParamsSchema = z
  .object({
    ...
    user_keys: z
      ...
      .max(100)
      .describe(
        "要批量查询的用户 user_key 列表（单次最多 100 个），如 ['7136000000000000676']",
      ),
  })
  .describe(
...
export type GetUsersByUserKeysParams = z.infer<typeof GetUsersByUserKeysParamsSchema>;

/** 获取单个用户的请求参数 */
export const GetUserParamsSchema = z
  .object({
    /** 用户名称、邮箱或 user_key */
    query: z.string().min(1).describe("user_key、名称或邮箱"),
    /** 空间 ID（可选，用于判断所属租户） */
    project_key: z
      .string()
      .optional()
  ...
  .describe("通过 user_key、名称或邮箱获取单个用户详情。");
...
export const SearchUsersParamsSchema = z
  .object({
    ...
    query: z
      .string()
      .min(1)
      .describe("支持名称（包含中英文）、邮箱或 user_key 精确或模糊匹配"),
    /** 空间 ID（可选，用于判断所属租户） */
    project_key: z
      .string()
      .optional()
